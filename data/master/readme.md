...
816,791 seats across 12 states, in one declared schema. One file per state; the headers are identical, so concatenating them is the whole table.

## What a row is

**One row per seat as a source document states it** — which is not the same as one row per seat, and the master does not claim it is. 806,222 rows (98.7%) identify a distinct seat; the other 10,569 are in [master_key_collisions.csv](master_key_collisions.csv) rather than hidden behind a promise the data does not support.
...
| [Jharkhand](master_jharkhand.csv.gz) | 20,917 |
...
- **page** — 235,975 rows, traceable to the page it was read from
...
| [`master_jharkhand.csv.gz`](master_jharkhand.csv.gz) | 20,917 | one state, one row per seat |
...
| [`master_key_collisions.csv`](master_key_collisions.csv) | 10,569 | rows that do not identify a distinct seat |
