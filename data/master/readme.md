...
815,736 seats across 12 states, in one declared schema. One file per state; the headers are identical, so concatenating them is the whole table.

## What a row is

**One row per seat as a source document states it** — which is not the same as one row per seat, and the master does not claim it is. 805,642 rows (98.8%) identify a distinct seat; the other 10,094 are in [master_key_collisions.csv](master_key_collisions.csv) rather than hidden behind a promise the data does not support.
...
- **page** — 234,920 rows, traceable to the page it was read from
...
| [`master_key_collisions.csv`](master_key_collisions.csv) | 10,094 | rows that do not identify a distinct seat |
