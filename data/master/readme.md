...
822,429 seats across 12 states, in one declared schema. One file per state; the headers are identical, so concatenating them is the whole table.

## What a row is

**One row per seat as a source document states it** — which is not the same as one row per seat, and the master does not claim it is. 815,223 rows (99.1%) identify a distinct seat; the other 7,206 are in [master_key_collisions.csv](master_key_collisions.csv) rather than hidden behind a promise the data does not support.
...
| [Jharkhand](master_jharkhand.parquet) | 26,555 |
...
- **page** — 241,613 rows, traceable to the page it was read from
...
| [`master_jharkhand.parquet`](master_jharkhand.parquet) | 26,555 | one state, one row per seat |
...
| [`master_key_collisions.csv`](master_key_collisions.csv) | 7,206 | rows that do not identify a distinct seat |
