...
| `source_capture` | string | info | length 8–20 | The web archive's capture timestamp, YYYYMMDDhhmmss. With source_url this refetches the exact bytes the row was read from - a live URL may since have changed or gone. Blank for documents that were not fetched from an archive. |
