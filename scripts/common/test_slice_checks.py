...
import slice_checks as sc

ROOT = "."


def make(rows, state="Goa", year="2012", tier="gp_ward"):
    base = {"state": state, "year": year, "tier": tier, "tier_local": "ward",
            "caste_reservation": "NONE", "woman_reserved": "0",
            "reservation": "Other than Woman", "district": "North Goa",
            "block": "Bardez", "gram_panchayat": "Anjuna", "ward_no": "1",
            "source_path": "readme.md", "source_page": "1"}
...
def test_a_category_outside_the_declared_scheme_fails():
    ...
    got = sc.caste_scheme_respected(
        make([{"caste_reservation": "BC"}], state="Kerala"))
    assert got["status"] == sc.FAIL


def test_a_scheme_with_no_backward_class_passes_on_absence():
    got = sc.caste_scheme_respected(
        make([{"caste_reservation": "SC"}], state="Kerala"))
    assert got["status"] == sc.PASS
...
def test_a_floor_is_not_a_target():
    ...
    got = sc.women_share_vs_statute(
        make(rows, state="Jammu & Kashmir", year="2010"))
    assert got["status"] == sc.PASS
...
def test_the_label_must_agree_with_the_two_fields_it_summarises():
    got = sc.label_agrees(make([{"caste_reservation": "SC",
