...
def render_parsed(directory, state, entries):
    ...
    out.append(f"Parses from the committed documents and rewrites the files "
               f"above, then runs `scripts/{directory}/validate.py`.")
    out.append("")
...
def render_unparsed(directory, state, status, where):
    ...
    out.append("See [../../SOURCES.md](../../SOURCES.md) for where this state "
               ...
               "A parser lands in `scripts/<state>/parse.py`, writes through "
               "`scripts/common/emit.py` so every row carries its source "
